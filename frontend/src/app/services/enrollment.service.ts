...
export class EnrollmentService {
  ...
  getEnrollment(courseId: string): Observable<Enrollment> {
    return this.http.get<Enrollment>(`${this.apiUrl}/${courseId}`);
  }

  updateProgress(courseId: string, lessonId: string): Observable<Enrollment> {
    return this.http.put<Enrollment>(`${this.apiUrl}/${courseId}/progress`, { lessonId });
  }
}
